...
Both Vercel projects deploy from the monorepo root with `vercel.json`. The `/api` function forwards requests to the project's `API_ORIGIN` environment variable, keeping browser sessions on the frontend origin. Each Railway API service uses the root `Dockerfile`, its environment's PostgreSQL `DATABASE_URL`, `NODE_ENV=production`, and an exact `WEB_ORIGIN` matching its frontend. Railway's staging and production service settings both gate deployments on `/health` (120-second timeout) and restart failed instances. Never point staging at the production database.
